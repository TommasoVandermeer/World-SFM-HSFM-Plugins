...
#ifndef GAZEBO_PLUGINS_WORLDMHSFMPLUGIN_HH_
#define GAZEBO_PLUGINS_WORLDMHSFMPLUGIN_HH_
...
// Social Force Model
#include <world_sfm_hsfm_plugins/mhsfm.hpp>

namespace gazebo {
class GZ_PLUGIN_VISIBLE WorldMHSFMPlugin : public WorldPlugin {
...
private:
  ...
  /// \brief Variable to store agent Ci
  std::vector<double> agentCi;

  /// \brief Variable to store agent Cw
  std::vector<double> agentCw;

  /// \brief Variable to store agent Di
  std::vector<double> agentDi;

  /// \brief Variable to store agent Dw
  std::vector<double> agentDw;
